...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($action === 'create') {
    ...
    header('Location: '.$_SERVER['PHP_SELF'].'?'.($ok ? 'ok=1' : 'err='.urlencode('Greška pri upisu')));
    exit;
  }
  ...
  if ($action === 'update') {
    ...
    if ($id <= 0 || $data['ime'] === '' || $data['prezime'] === '') {
      header('Location: '.$_SERVER['PHP_SELF'].'?err='.urlencode('Neispravan unos'));
      exit;
    }
    $ok = $model->update($id, $data);
    header('Location: '.$_SERVER['PHP_SELF'].'?'.($ok ? 'ok=1' : 'err='.urlencode('Greška pri izmeni')));
    exit;
  }
}
